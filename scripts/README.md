...
## `analyze_summary_lengths.py`

Analisi **per cluster** delle lunghezze dei riassunti generati dai 18 metodi del benchmark
(issue #15). Risponde alla domanda che le medie aggregate per metodo non coprono: dentro un
singolo gruppo Multi-News, quanto sono diversi fra loro i riassunti dei vari metodi, e come si
rapportano al riassunto di riferimento di quel gruppo?

### Uso

```
python scripts/analyze_summary_lengths.py                      # ambito test (default)
python scripts/analyze_summary_lengths.py --tabella-budget     # ricalcola T(n) sulla split train
python scripts/analyze_summary_lengths.py --scope test_budget  # la corsa "dopo" il contenimento
python scripts/analyze_summary_lengths.py --figure results/figures/analisi_lunghezze
```

Dipendenze: `numpy` sempre, `matplotlib` solo con `--figure`. Mettere in conto ~1 minuto: il
passaggio lento è la singola scansione in streaming di `data/tab/complete.tab`.

### Input

- I `results/metrics/{metodo}_{scope}_per_example.csv` già committati, da cui legge la colonna
  `parole_generate` — **nessun riassunto viene rigenerato e nessuna metrica ricalcolata**. I file
  G-Eval non corrispondono al pattern e restano quindi esclusi da soli.
- `data/tab/complete.tab` in streaming, per la lunghezza del riassunto di riferimento
  (`su.pulisci_riferimento` poi `str.split()`) e il numero di articoli di ogni cluster. Il conteggio
  degli articoli è fatto sul documento **grezzo**, contando il separatore `|||||`: dopo
  `prepara_documento` il separatore è diventato un newline e non è più distinguibile dai newline
  interni agli articoli.

L'analisi è ristretta ai `row_id` coperti da **tutti** i metodi trovati (5.437 sulla split test),
la stessa logica di intersezione del notebook 05.

### Output

- `results/metrics/analisi_lunghezze_{scope}.json` (committato) — dispersione per cluster,
  statistiche per metodo, tabella del budget e distribuzione del riferimento.
- `scripts/budget_lunghezza.json` (committato) — la tabella `T(n_articoli)`, scritta con
  `--tabella-budget` o automaticamente se manca.
- Con `--figure`, tre PNG: boxplot del rapporto col riferimento per metodo, istogramma del
  rapporto max/min dentro il cluster, scatter riferimento vs lunghezza generata.

### Che cosa fa

- **Dispersione dentro ciascun cluster** — spread (max − min), rapporto max/min, coefficiente di
  variazione, e spread rapportato alla lunghezza del riferimento, riassunti in percentili.
- **Per metodo** — distribuzione di `lunghezza/riferimento` e di `lunghezza/T`, quota di cluster
  dentro la banda `[0,80 · T, 1,25 · T]`, e correlazione di Pearson fra lunghezza generata,
  lunghezza del riferimento e numero di articoli (misura quanto il metodo si *adatta* al cluster).
- **`T(n_articoli)`** — mediana della lunghezza del riferimento per numero di articoli, con
  bucket `1..7` e `8+`. È il budget in parole per-cluster usato dal contenimento delle lunghezze.
  È tarata sulla split **train**, non su test: il budget non deve incorporare informazione dalle
  etichette su cui poi si misura.

### Perché un budget per-cluster e non un tetto globale

La lunghezza del riassunto di riferimento non è costante: cresce col numero di articoli del
cluster, da ~164 parole (1 articolo) a ~327 (8 o più). Un tetto unico uguale per tutti taglierebbe
troppo i cluster grandi e non vincolerebbe quelli piccoli. Da qui `T(n_articoli)`.

### Quando rilanciarlo

Dopo ogni corsa che produce un nuovo ambito di metriche (`--scope`), per confrontare la
dispersione prima e dopo. `--tabella-budget` serve invece solo se cambia `data/tab/complete.tab`
o il criterio di taratura: la tabella è un **parametro** del benchmark, non un output da
rigenerare a ogni esecuzione.
